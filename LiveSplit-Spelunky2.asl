...
update
{
  ...
  } else if(settings["tiamat"] && current.world == 6 && current.screen == 16 && old.screen != 16) {
    print("Setting delayed split after Tiamat at "+(current.counter+1).ToString());
    vars.splitAt = current.counter+1;
  } else if(settings["hundun"] && current.world == 7 && current.screen == 16 && old.screen != 16) {
    print("Setting delayed split after Hundun at "+(current.counter+1).ToString());
    vars.splitAt = current.counter+1;
  } else if(settings["co"] && current.screen == 19 && old.screen != 19) {
